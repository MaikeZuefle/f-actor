...
def main(input_dir, output_dir, num_pairs=5, seed=None):
    ...
    with torch.no_grad():
        ...
        for idx, c1_path in enumerate(selected, 1):
            ...
            c1_length = torch.tensor(c1.shape[-1]).unsqueeze(0).to(device)

            c2 = torch.from_numpy(np.load(c2_path)).to(device)
            c2_length = torch.tensor(c2.shape[-1]).unsqueeze(0).to(device)

            audio_c1 = (
                model.decode(tokens=c1.unsqueeze(0), tokens_len=c1_length)[0]
                .cpu()
                .numpy()
            )
            audio_c2 = (
                model.decode(tokens=c2.unsqueeze(0), tokens_len=c2_length)[0]
                .cpu()
                .numpy()
            )
